...
import {
  Box, Button, Paper, TableContainer, Table, TableHead, TableRow, TableCell, TableBody, Typography,
  Tooltip, IconButton, Dialog, DialogActions, DialogContent, DialogTitle, List, ListItem, ListItemText
} from '@mui/material';
  ...
  const handleOpenModal = (day, timeSlot) => {
    ...
    const filteredClasses = availableClasses.filter(classData => {
      const matchesTimeSlot = classData.schedules.some(schedule => {
        ...
        return mappedDay === day && schedule.start === timeSlot.split('-')[0] && schedule.end === timeSlot.split('-')[1];
      });
      ...
      <Paper sx={{ padding: 2, backgroundColor: theme === 'light' ? '#ffffff' : '#424242', color: theme === 'light' ? '#000000' : '#ffffff' }}>
        ...
        <TableContainer>
          <Table>
            <TableHead>
              <TableRow>
                <TableCell sx={{ width: '120px', backgroundColor: theme === 'light' ? '#e0e0e0' : '#616161', color: theme === 'light' ? '#000000' : '#ffffff' }}>
                  Horário
                </TableCell>
                {days.map((day) => (
                  <TableCell
                    key={day}
                    sx={{ width: '200px', textAlign: 'center', backgroundColor: theme === 'light' ? '#e0e0e0' : '#616161', color: theme === 'light' ? '#000000' : '#ffffff' }}
                  >
                    {day}
                  </TableCell>
                ))}
              ...
              {timeSlots.map((timeSlot) => (
                <TableRow key={timeSlot}>
                  ...
                  {days.map((day) => {
                    ...
                    return (
                      <Tooltip title={classInfo ? "Clique para remover a disciplina da agenda" : "Clique para adicionar uma disciplina"} key={day}>
                        <TableCell
                          sx={{
                            width: '200px',
                            maxWidth: '200px',
                        ...
                        >
                          {classInfo ? (
                            <Typography variant="subtitle2" noWrap sx={{ color: theme === 'light' ? '#000000' : '#ffffff' }}>
        ...
        <Dialog
            ...
            >
            <DialogTitle sx={{ backgroundColor: theme === 'light' ? '#ffffff' : '#424242', color: theme === 'light' ? '#000000' : '#ffffff' }}>
            ...
            <DialogContent sx={{ backgroundColor: theme === 'light' ? '#ffffff' : '#424242', color: theme === 'light' ? '#000000' : '#ffffff' }}>
                ...
                {availableClassesForSlot.length > 0 ? (
                    ...
                    <ListItem button key={classData.turma} onClick={() => handleAddClass(classData)}>
                        <ListItemText primary={`${classData.nome_da_disciplina} (${classData.turma})`} sx={{ color: theme === 'light' ? '#000000' : '#ffffff' }} />
                    </ListItem>
                    ))
                ) : (
                    <Typography sx={{ color: theme === 'light' ? '#000000' : '#ffffff' }}>Nenhuma disciplina disponível neste horário.</Typography>
                )}
                </List>
            </DialogContent>
            <DialogActions sx={{ backgroundColor: theme === 'light' ? '#ffffff' : '#424242' }}>
                <Button onClick={handleCloseModal} sx={{ color: theme === 'light' ? '#000000' : '#ffffff' }}>Cancelar</Button>
            </DialogActions>
        </Dialog>
      </Paper>
