  ...
  const handleToggleClass = useCallback(
    (classData) => {
      ...
      if (hasSameSubjectConflict) {
        showSnackbar(
          `Já existe uma turma da disciplina ${classData.nome_da_disciplina} na sua agenda!`,
          "warning",
        );
        return;
      }
          ...
          <Table stickyHeader>
            ...
            <TableBody>
              {filteredClasses.map((classData, index) => {
                ...
                const hasTimeConflict = classData.schedules?.some(
                  (scheduleItem) => {
                    const key = `${scheduleItem.day} ${scheduleItem.start}-${scheduleItem.end}`;
                    return (
                      schedule[key] &&
                      schedule[key].subjectName !== classData.nome_da_disciplina
                    );
                  },
                );
